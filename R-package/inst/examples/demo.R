# Ejemplos de uso de iberianbirds con ggplot2
library(ggplot2)
...
ggplot(df, aes(cat, val, fill = cat)) +
  geom_col() +
  scale_fill_iberianbirds("manchego") +
  labs(title = "iberianbirds — manchego (cualitativa)") +
  theme_minimal()
...
ggplot(faithful, aes(waiting, eruptions, color = eruptions)) +
  geom_point(size = 2) +
  scale_color_iberianbirds_c("rabilargo_seq") +
  theme_minimal()

# 4. Continua divergente (naranja-azul del martin pescador)
ggplot(mtcars, aes(wt, mpg, color = scale(drat)[, 1])) +
  geom_point(size = 3) +
  scale_color_iberianbirds_c("martin_div") +
  labs(color = "drat (z)") +
  theme_minimal()

# 5. Ver una paleta
ib_mostrar("avefria")
ib_mostrar("flamenco_seq")
